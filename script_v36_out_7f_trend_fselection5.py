...
fimp = pd.read_csv('2level/fimp_all.csv')
fimp.drop('Unnamed: 0',axis=1,inplace=True)
fimp.set_index('f',drop=True,inplace=True)

fimpr = pd.read_csv('2level_random/fimp_all.csv')
fimpr.drop('Unnamed: 0',axis=1,inplace=True)
fimpr.set_index('f',drop=True,inplace=True)
...
selection_df['f']=fimp.index.values
for f in fimp.index.values:
    tres = ttest_ind(fimp.loc[f], fimpr.loc[f])
    pvalues.append(tres[1])
    fc.append(fimp.loc[f].mean()/ fimpr.loc[f].mean())
selection_df['pvalues']=pvalues
selection_df['fc']=fc
selection_df['log2fc']=np.log2(selection_df['fc'])
selection_df['log10pvalues']=-np.log10(selection_df['pvalues'])
padj = multipletests(selection_df['pvalues'], method='bonferroni')
selection_df['padj']=padj[1]
selection_df.to_csv(OUT+'fselection.csv')

to_use = list(selection_df[selection_df['padj']<0.05]['f'])
df = df[to_use+['TARGET']]
